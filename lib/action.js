...
function collectPromisesFromArguments(args) {
	var promises,
		index = 0,
		arg;
...
function replacePromiseArguments(args, callback) {
	var promises = collectPromisesFromArguments(args);

	// If there are no promises found in the list of args,
	// we can just return immediately. We don't have to return
	// the args explicitly eg "callback(null, args)" because our
	// reference to args points to the same object in memory as the
	// consumer that called this function (we just operate destructively).
	if (!promises) {
	...
	whenAllPromisesResolve(promises, function handler(error, results) {
		...
		while (index < args.length) {
			...
			if (isPromise(arg)) {
				args[index] = results.shift();
			}

			index++;
		}

		// See note above about why we're not actually returning the args.
		return void callback(null);
	});
}
...
Action.prototype.dispatch = function dispatch() {
	...
	replacePromiseArguments(this.args, bind(this.handlePromisesResolved, this));
};
