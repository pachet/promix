...
PrivateMethods: {
	...
	Chain.prototype.addStep = function addStep() {
		...
		if (!this.hasInsertionStep()) {
			step = this.addStepAtIndex(0);
		} else {
			step = this.addStepAfter(this.getInsertionStep());
		}
	...
	Chain.prototype.addStepIfNecessary = function addStepIfNecessary() {
	...
	Chain.prototype.addStepAfter = function addStepAfter(step) {
		var index = this.getIndexOfStep(step);

		return this.addStepAtIndex(index + 1);
	};

	Chain.prototype.addStepBefore = function addStepBefore(step) {
		var index = this.getIndexOfStep(step);

		return this.addStepAtIndex(index);
	};
	...
	Chain.prototype.getCurrentStepIndex = function getCurrentStepIndex() {
		return this.getIndexOfStep(this.getCurrentStep());
	};

	Chain.prototype.getIndexOfStep = function getIndexOfStep(step) {
	...
	Chain.prototype.hasInsertionStep = function hasInsertionStep() {
		return this.getInsertionStep() !== null;
	};

	Chain.prototype.getInsertionStepIndex = function getInsertionStepIndex() {
		return this.getIndexOfStep(this.getInsertionStep());
	};
...
PublicMethods: {
	...
	Chain.prototype.andCall = function andCall(fn) {
		...
		if (arguments.length > 1) {
			throw new Error(
				'Wrong number of arguments to chain.andCall()'
			);
		}
	...
	Chain.prototype.then = function then() {
		this.addStepIfNecessary();
		return this.and.apply(this, arguments);
	};

	Chain.prototype.andSync = function andSync() {
		var args = slice(arguments),
			fn = args.shift();

		if (typeof fn !== 'function') {
			throw new Error(
				'First argument to chain.thenSync() must be a function'
			);
		}

		args.unshift(function injectedAndSync() {
			var args = slice(arguments),
				callback = args.pop(),
				result = undefined;
	...
	Chain.prototype.thenSync = function thenSync() {
		this.addStepIfNecessary();
		return this.andSync.apply(this, arguments);
	};

	Chain.prototype.thenCall = function thenCall() {
		this.addStepIfNecessary();
		return this.andCall.apply(this, arguments);
	};
	...
	Chain.prototype.delay = Chain.prototype.sleep;
	...
	Chain.prototype.bind = function bind(context) {
		...
		if (!insertion_step.isIteration()) {
			if (insertion_step.hasFailureAction()) {
				...
				if (insertion_step.actions.length === 0) {
					throw new Error('wat');
				}
		...
		var iteration_steps = this.getCurrentIterationSteps();

		iteration_steps.forEach(function each(step) {
	...
	Chain.prototype.each = function each(array, fn, sequential) {
		...
		if (isPromise(array)) {
			method.call(this, function interstitial(results, callback) {
				...
				var
					current_step = self.getInsertionStep(),
					new_step;

				if (current_step === prior_step) {
					new_step = self.addStepAfter(prior_step);
				} else {
					new_step = self.addStepBefore(prior_step);
				}

				self.setInsertionStep(new_step);
				self.each(results, fn, sequential).bind(this);
				self.setInsertionStep(current_step);

				callback(null);
			}, array).omit();

			prior_step = this.getInsertionStep();
