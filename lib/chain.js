...
Chain.prototype.complete = function complete ( step ) {
	...
	if ( length > 0 ) {
		while ( index < length ) {
			...
			if ( !step.expunged && !step [index].expunged ) {
			...
			if ( typeof step [index].labels !== 'undefined' ) {
				...
				while ( labels.length ) {
					...
					if ( !step.property_expunged && !step[index].property_expunged ) {
						this.results [label] = currentResult;
					}
			...
			if ( step.condition ) {
				if ( currentResult === step.conditionValue ) {
					conditionMatch = true;
				}
			}
...
Chain.prototype.applyHooks = function applyHooks ( step, subIndex ) {
	...
	if ( !step.condition && promise.dispatched ) {
		return;
	}
...
Chain.prototype.expunge = function expunge ( index, expunge_property ) {
	var
		stepItem,
		reflector;
	...
	if ( stepItem.length === 0 ) {
		stepItem.expunged = true;
		if ( expunge_property ) {
			stepItem.property_expunged = true;
		}
	}
	else {
		stepItem = stepItem [stepItem.length - 1];
