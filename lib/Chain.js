...
function Chain ( ) {
	...
	this.exports = {
		...
		then : function then ( interlocutor, secondary ) {
			...
			if ( type === 'undefined' || ( type === 'function' && arguments.length === 2 ) ) {
				step.then = interlocutor;
				if ( type === 'function' ) {
					self.exports.otherwise(secondary);
				}
			}
			else {
				self.add_promise.apply(self, Array.prototype.slice.call(arguments));
		...
		otherwise : function otherwise ( ) {
			...
			self.steps [self.focus].handler = args [0];
			if ( args.length > 1 ) {
				self.steps [self.focus].error_arguments = args.slice(1);
			}
			return self.exports;
		},
				...
				if ( current_introspect && typeof current_introspect.then === 'function' ) {
					...
					introspect_domain.then(function fulfill_introspect ( results ) {
						if ( promise.reflection ) {
							promise.reflection.returned = introspect_domain.introspect.returned;
						}
